...
func cachePath(dir string, a asset) string {
	return filepath.Join(dir, strings.TrimPrefix(a.Digest, "sha256:")+"-"+a.Name)
}

func effectiveCacheDir(cfg config) string {
	if cfg.CacheDir != "" {
		return cfg.CacheDir
	}
	return filepath.Join("/data/adb/cache", cfg.ModuleID)
}

func useCached(a asset, cfg config, out string) error {
	dir := effectiveCacheDir(cfg)
	cached := cachePath(dir, a)
	b, e := os.ReadFile(cached)
	if e != nil {
		return e
	}
	h := sha256.Sum256(b)
	if "sha256:"+hex.EncodeToString(h[:]) != a.Digest || int64(len(b)) != a.Size {
		_ = os.Remove(cached)
		return errors.New("stale or corrupt cache")
	}
	if e = checkZipBytes(b, cfg.ModuleID); e != nil {
		_ = os.Remove(cached)
		return e
	}
	...
	fmt.Println("Verified cache hit; skipping module download.")
	return nil
}

func checkZipBytes(b []byte, id string) error {
	f, e := os.CreateTemp("", "kam-module-cache-*.zip")
	if e != nil {
		return e
	}
	name := f.Name()
	...
	return checkZip(name, id)
}

func saveCached(a asset, cfg config, file string) {
	dir := effectiveCacheDir(cfg)
	...
	if e == nil {
		_ = os.Rename(tmpName, cachePath(dir, a))
	}
}
...
func runWithClient(cfg config, out string, c *http.Client) error {
	...
	if e = useCached(a, cfg, out); e == nil {
		return nil
	}
	tmp := out + ".partial"
	defer os.Remove(tmp)
